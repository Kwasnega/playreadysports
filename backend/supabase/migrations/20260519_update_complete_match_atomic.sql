-- Update complete_match_atomic to credit venue owner for free matches
-- When entry_fee = 0 but organizer_venue_fee > 0, the venue owner gets paid.
...
BEGIN
  -- Lock the match
  SELECT id, organizer_id, venue_id, status, entry_fee, match_mode, organizer_venue_fee
  INTO v_match
  FROM public.matches
  WHERE id = p_match_id
  FOR UPDATE;
  ...
  -- Free match: credit venue owner from organizer_venue_fee
  IF COALESCE(v_match.entry_fee, 0) = 0 AND COALESCE(v_match.organizer_venue_fee, 0) > 0 THEN
    DECLARE
    ...
    BEGIN
      ...
      IF v_venue_owner_id IS NOT NULL THEN
        ...
        SET venue_owner_balance = COALESCE(venue_owner_balance, 0) + v_match.organizer_venue_fee
        WHERE id = v_venue_owner_id;

        BEGIN
          INSERT INTO public.wallet_transactions (user_id, amount, type, status, reference)
          VALUES (v_venue_owner_id, v_match.organizer_venue_fee, 'venue_payout', 'completed', 'match_' || p_match_id);
        EXCEPTION WHEN undefined_table THEN NULL; END;
      END IF;
    END;
  ELSIF v_prize_pool > 0 THEN
    -- Paid match: credit organizer (existing behavior)
    UPDATE public.profiles
