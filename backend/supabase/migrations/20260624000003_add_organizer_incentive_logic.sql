-- ============================================================
-- Migration: 20260624000003_add_organizer_incentive_logic.sql
-- Fixes Issue 10: Organizer incentive dynamic amount and popup notification
-- ============================================================

-- 1. Add organizer_incentive to transaction_type enum
ALTER TYPE public.transaction_type ADD VALUE IF NOT EXISTS 'organizer_incentive';

-- 2. Update complete_match_atomic to read from platform_settings and create notification
...
BEGIN
  ...
  IF v_completed_count > 0 AND v_completed_count % 5 = 0 THEN
    -- Read dynamic incentive amount
    SELECT COALESCE((value::numeric), 0) INTO v_incentive_amount
    FROM public.platform_settings
    WHERE key = 'organizer_incentive';

    IF v_incentive_amount IS NULL OR v_incentive_amount <= 0 THEN
      -- Fallback if not set
      v_incentive_amount := 10.00;
    END IF;
    ...
    -- Insert notification to trigger the frontend popup
    INSERT INTO public.notifications (
      ...
      v_match.organizer_id,
      'Milestone Achieved!',
      '🎉 You''ve been credited GHS ' || v_incentive_amount || ' for completing 5 matches as an active organizer on PlayReady Sports!',
      'system',
      jsonb_build_object('type', 'organizer_incentive', 'amount', v_incentive_amount, 'completed_count', v_completed_count),
      false,
      now()
    );
