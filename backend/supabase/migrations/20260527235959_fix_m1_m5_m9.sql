-- ============================================================
-- Migration: 20260527235959_fix_m1_m5_m9.sql
--
-- Fixes:
--   M5 — Ensure every user has a wallet_balances row (back-fill
--         any users created before the trigger existed)
--   M9 — Win/loss stat update skips 'unassigned'/'spectator'
--         participants in complete_match_atomic
--
-- NOTE on M1: The C1 migration (20260527230000_fix_critical_bugs.sql)
-- already unified the wallet system — process_wallet_transaction reads
-- and writes wallet_balances directly. There is NO profiles.wallet_balance
-- column. The sync trigger is therefore NOT needed. The frontend already
-- reads from wallet_balances, which is correct.
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- M5: Back-fill wallet_balances rows for any users who were
-- created before the on_auth_user_created_wallet trigger existed.
...
-- ─────────────────────────────────────────────────────────────
-- M9 + H8: Rewrite complete_match_atomic with a 3-argument
-- signature so p_winning_team is accepted from the Edge Function,
-- the status gate accepts 'full' OR 'live' (H8 fix), and the
-- win/loss loop skips 'unassigned'/'spectator'/NULL team players.
-- ─────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Organizer completion bonus (loyalty credit)
  SELECT public.process_wallet_transaction(
    v_match.organizer_id,
    2.00,
    'bonus',
