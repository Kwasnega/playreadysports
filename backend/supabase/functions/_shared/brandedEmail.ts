...
export function venueOwnerPasswordResetEmail(
  to: string,
  fullName: string,
  password: string,
  ...
  const html = shell(`
    <p style="margin:0 0 20px;color:#2c3e50;font-size:16px;line-height:1.8;"><strong>Hello ${name},</strong></p>
    <p style="margin:0 0 20px;color:#555555;font-size:15px;line-height:1.7;">
      Your PlayReady <strong>Turf Owner</strong> account password has been reset by an administrator.
    </p>

    <div style="margin:24px 0;padding:20px;border-radius:8px;background:#fefce8;border:2px solid #eab308;">
      <div style="font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#a16207;font-weight:700;margin-bottom:12px;">🔑 Your New Login Credentials</div>
      <table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="padding:8px 0;color:#666666;font-size:14px;font-weight:600;width:90px;">Email</td>
          <td style="padding:8px 0;color:#2c3e50;font-size:14px;font-family:monospace;">${escapeHtml(to)}</td>
    ...
    <div style="margin:20px 0;padding:16px;border-radius:8px;background:#fef2f2;border-left:4px solid #ef4444;">
      <p style="margin:0;color:#991b1b;font-size:13px;line-height:1.6;">
        <strong>⚠️ Security:</strong> Please change your password immediately after logging in. Do not share these credentials with anyone.
      </p>
    </div>
    \` : ''}
  `, "Your PlayReady Turf Owner account password has been reset.", \`\${brand.origin}/venue/dashboard\`, "Go to Your Dashboard");

  return {
    to,
    subject: "Your PlayReady Password Has Been Reset 🔐",
    html,
    text: \`Hello \${fullName || "Turf Owner"}! Your PlayReady password has been reset. Email: \${to} | New Password: \${password} | Dashboard: \${brand.origin}/venue/dashboard \${isAutoGenerated ? '— Please change your password after logging in.' : ''}\`,
  };
}
